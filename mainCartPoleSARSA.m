...
QFunction = getQFunctionLookUpTable( environment, 'discretization', [2,2,8,2], 'eligibilitytrace', 1 );
...
if (length(stepNumTest)>98)
    for i1 = 1:length(history)
        if (isempty(history{i1})), break, end
        plot_Cart_Pole( history{i1}.values , environment.avalues(history{i1}.action.actionIdx(1)) ,i1 )
    end
end
return;
